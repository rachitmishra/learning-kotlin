...
class Customer(var id: Int, var name: String = "My name") : Any() { /* initialising class with default value */
    ...
    @JvmOverloads fun changeStatus(status: Status = Status.APPROVED) {  /* Using @JvmOverloads we can create overloaded version of the method*/
        
    }
    
    @JvmOverloads @JvmName("preferential") fun makePreferred(status: Status = Status.PENDING) {  /* Using @JvmName we can change the name of the method and how it can be accessed from Java*/
